...
} from "./baseballMarketUtils";

interface BaseballGameDetailsModalProps {
  open: boolean;
  onClose: () => void;
  game: BaseballGameWithPrediction | null;
}

const BaseballGameDetailsModal: React.FC<BaseballGameDetailsModalProps> = ({
  open,
  onClose,
  game,
}) => {
  ...
  const activeCategory = BASEBALL_CATEGORY_TABS[activeTab];
  const activeMarkets = groupedMarkets.get(activeCategory?.key) || [];
  const availableTabs = BASEBALL_CATEGORY_TABS.filter(
    (tab) => (groupedMarkets.get(tab.key) || []).length > 0
  );

  const renderMarketRow = (market: BaseballMarket) => {
      ...
      <DialogTitle sx={{ pb: 1 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Chip
              label={game.day_night === "night" ? "Noche" : "Día"}
              size="small"
              sx={{ bgcolor: game.day_night === "night" ? "#6366f1" : "#f59e0b", color: "white", fontWeight: 600 }}
            />
          </Box>
...
export default BaseballGameDetailsModal;
