...
def _mock_training_services():
    ...
    class MockRepo:
        ...
        def save_binary_artifact_versioned(self, key, version, data, meta):
            artifacts[self._versioned_doc_key(key, version)] = {
                "data": data,
                "meta": meta,
            }
        ...
        def promote_serving_pointer(
            self, pointer_key=pointer_key, artifact_key="", version="", metrics=None
        ):
            app_state[pointer_key] = {
    ...
    class MockRes:
        def resolve_pick(self, *a, **k):
            return MagicMock()

    repo = MockRepo()
    # Pre-seed a serving model V1
    v1_bytes = _make_model_bytes(_DummyRF())
    repo.save_binary_artifact_versioned(
        "models/picks_classifier",
        "v1",
        v1_bytes,
        {
...
async def test_failed_run_keeps_prior_model_serving():
    ...
    with patch("src.application.services.ml_evaluation_gate.run_gate") as mock_gate:
        ...
        with patch(
            "src.application.services.ml_training_orchestrator.prepare_datasets",
...
async def test_full_roundtrip_gate_pass_promotes_new_version():
    ...
    with patch("src.application.services.ml_evaluation_gate.run_gate") as mock_gate:
        ...
        with patch(
            ...
            await orchestrator.run_training_pipeline(league_ids=["L1"], days_back=10)
    ...
    repo.save_binary_artifact_versioned(
        ...
        v2_bytes,
        {
            ...
            "git_sha": "abc12345",
            "trained_at": "2024-01-01T00:00:00Z",
            "metrics": {"log_loss": 0.4},
        },
    )
    repo.promote_serving_pointer(
        artifact_key="models/picks_classifier", version="v2", metrics={"log_loss": 0.4}
    )
    ...
    assert "models/picks_classifier/v2" in artifacts
