...
def get_prediction_service() -> BasketballPredictionService:
    ...
    if _prediction_service is None:
        ...
        _prediction_service = BasketballPredictionService(
...
async def predict_basketball_game(request: BasketballPredictRequest) -> BasketballPredictResponse:
...
@router.get("/games")
async def get_upcoming_games(team: Optional[str] = Query(None)) -> BasketballGamesResponse:
    """Get upcoming basketball games."""
    try:
        all_games = get_all_games()
        if not all_games:
            return BasketballGamesResponse(
                games=[], generated_at=date.today().isoformat()
            )
        if team:
            team = team.upper()
            all_games = [
        ...
        conferences = [
            {
                ...
                "teams": BasketballFeatureExtractor.CONFERENCES["East"],
            },
            {
                "id": "west",
                "name": "Western Conference",
                "teams": BasketballFeatureExtractor.CONFERENCES["West"],
            },
        ]
...
async def get_conference_predictions(conference_id: str) -> BasketballConferenceGamesResponse:
    """Get predictions for games in a specific conference."""
    try:
        all_games = get_all_games()
        conf_teams = BasketballFeatureExtractor.CONFERENCES.get(
            conference_id.capitalize(), []
        )
        if not conf_teams:
            raise HTTPException(
                status_code=404, detail=f"Conference '{conference_id}' not found"
            )
