...
class BaseballPredictionService:
    ...
    def _predict_with_model(self, features: dict, feature_names: list,
                            game: BaseballGame) -> Optional[BaseballPrediction]:
    ...
    def _predict_rule_based(self, features: dict, game: BaseballGame) -> BaseballPrediction:
        ...
        home_strength = (home_wr * 0.4) + ((home_rsg / (home_rsg + away_rsg + 0.01)) * 0.3) + ((1 - home_ra / (home_ra + away_ra + 0.01)) * 0.3)
        away_strength = (away_wr * 0.4) + ((away_rsg / (home_rsg + away_rsg + 0.01)) * 0.3) + ((1 - away_ra / (home_ra + away_ra + 0.01)) * 0.3)
    ...
    def _get_form_context(self, game: BaseballGame) -> dict:
        ...
        fe = self.feature_extractor
        home_10 = fe._get_recent_team_stats(game.home_team, window=10, before_date=game.date)
        away_10 = fe._get_recent_team_stats(game.away_team, window=10, before_date=game.date)
        home_20 = fe._get_recent_team_stats(game.home_team, window=20, before_date=game.date)
        away_20 = fe._get_recent_team_stats(game.away_team, window=20, before_date=game.date)
        return {
    ...
    def _calculate_value_bets(self, game: BaseballGame,
    ...
    def _extract_key_factors(self, features: dict, game: BaseballGame) -> list:
        ...
        away_wr = features.get("away_win_rate_10", 0.5)
        if home_wr > 0.7:
            factors.append(f"{game.home_team} en racha ({home_wr*100:.0f}% en últimos 10)")
        if away_wr > 0.7:
            factors.append(f"{game.away_team} en racha ({away_wr*100:.0f}% en últimos 10)")

        # Run differential
        run_diff = features.get("run_diff_diff", 0)
        if abs(run_diff) > 1.0:
            better = game.home_team if run_diff > 0 else game.away_team
            factors.append(f"{better} tiene mejor diferencia de carreras (+{abs(run_diff):.1f})")
        ...
        if h2h_total >= 5:
            ...
            if h2h_home > 0.65:
                factors.append(f"{game.home_team} domina el H2H ({int(h2h_home*h2h_total)}/{h2h_total})")
            elif h2h_home < 0.35:
                factors.append(f"{game.away_team} domina el H2H ({int((1-h2h_home)*h2h_total)}/{h2h_total})")
        ...
        if features.get("pitcher_matchup_known"):
            ...
            if home_era < 3.0:
                factors.append(f"{game.home_pitcher_name or game.home_team} tiene ERA bajo ({home_era:.2f})")
            if away_era < 3.0:
                factors.append(f"{game.away_pitcher_name or game.away_team} tiene ERA bajo ({away_era:.2f})")

        return factors[:5]

    def generate_baseball_markets(self, game: BaseballGame,
                                  home_prob: float, away_prob: float) -> list:
        ...
        markets.append(_m("moneyline", f"{winner_team} gana", winner_prob,
        ...
        if rl_home > 0.4:
            markets.append(_m("run_line", f"{game.home_team} -1.5", rl_home, "RL_H", 0.5))
        if rl_away > 0.4:
            markets.append(_m("run_line", f"{game.away_team} +1.5", rl_away, "RL_A", 0.5))
        ...
        for thr in [7.5, 8.5, 9.5]:
            ...
            if thr > 9.0: base -= 0.1
            elif thr < 8.0: base += 0.1
            over_p = max(0.1, min(base, 0.9))
            markets.append(_m("total_runs_over", f"Más de {thr} carreras", over_p,
                              f"OU{int(thr*10)}_O", 0.55, 0.6))
            markets.append(_m("total_runs_under", f"Menos de {thr} carreras", 1-over_p,
        ...
        f5_a = 1.0 - f5_h
        if f5_h > 0.55:
            markets.append(_m("first_5_innings", f"{game.home_team} gana F5", f5_h, "F5_H", 0.6, 0.6))
        if f5_a > 0.55:
            markets.append(_m("first_5_innings", f"{game.away_team} gana F5", f5_a, "F5_A", 0.6, 0.6))
        ...
        for t_name, is_h in [(game.home_team, True), (game.away_team, False)]:
            ...
            for ou_thr in [3.5, 4.5]:
                tt_over = 0.7 if t_total > ou_thr + 1 else (0.55 if t_total > ou_thr else 0.35)
                markets.append(_m("team_total_runs", f"{t_name} más de {ou_thr} carreras", tt_over,
                                  f"TT{'H' if is_h else 'A'}_{int(ou_thr*10)}", 0.55, 0.6))

        # 6. Both Teams Score
        bts = min(0.5 + closer * 0.35, 0.9)
        markets.append(_m("both_teams_score", "Ambos equipos anotan", bts, "BTS_Y", 0.6, 0.7))
        markets.append(_m("both_teams_score_no", "Al menos uno no anota", 1-bts, "BTS_N", 0.6, 0.7))
