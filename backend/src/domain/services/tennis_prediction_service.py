...
class TennisPrediction:
    ...
    def __init__(self, p1_win_prob: float, p2_win_prob: float, confidence: float,
                 h2h: dict = None, surface_stats: dict = None, form: dict = None,
                 value_bets: list = None, key_factors: list = None):
...
class TennisPredictionService:
    ...
    def _get_surface_stats(self, match: TennisMatch) -> dict:
        ...
        fe = self.feature_extractor
        return {
            "p1_surface_win_rate": fe._get_surface_win_rate(match.p1_name, match.surface, match.match_date),
            "p2_surface_win_rate": fe._get_surface_win_rate(match.p2_name, match.surface, match.match_date),
    ...
    def _get_form_context(self, match: TennisMatch) -> dict:
        ...
        fe = self.feature_extractor
        p1_5 = fe._get_rolling_stats(match.p1_name, window=5, before_date=match.match_date)
        p2_5 = fe._get_rolling_stats(match.p2_name, window=5, before_date=match.match_date)
        p1_10 = fe._get_rolling_stats(match.p1_name, window=10, before_date=match.match_date)
        p2_10 = fe._get_rolling_stats(match.p2_name, window=10, before_date=match.match_date)
        return {
    ...
    def _calculate_value_bets(self, match: TennisMatch, p1_prob: float, p2_prob: float) -> list:
    ...
    def _extract_key_factors(self, features: dict, match: TennisMatch) -> list:
        ...
        if abs(rank_diff) > 3:
            ...
            factors.append(f"{better} tiene mejor ranking por {abs(rank_diff)} posiciones")
        
        # Surface dominance
        p1_surface = features.get("p1_surface_win_rate", 0.5)
        p2_surface = features.get("p2_surface_win_rate", 0.5)
        if p1_surface > 0.7:
            factors.append(f"{match.p1_name} domina en {match.surface} ({p1_surface*100:.0f}% victorias)")
        if p2_surface > 0.7:
            factors.append(f"{match.p2_name} domina en {match.surface} ({p2_surface*100:.0f}% victorias)")
        
        # Recent form
        p1_form = features.get("p1_win_rate_10", 0.5)
        p2_form = features.get("p2_win_rate_10", 0.5)
        if p1_form > 0.8:
            factors.append(f"{match.p1_name} en racha ({p1_form*100:.0f}% en últimos 10)")
        if p2_form > 0.8:
            factors.append(f"{match.p2_name} en racha ({p2_form*100:.0f}% en últimos 10)")
        ...
        if h2h_total >= 3:
            ...
            if h2h_p1 > 0.7:
                factors.append(f"{match.p1_name} domina el H2H ({int(h2h_p1*h2h_total)}/{h2h_total})")
            elif h2h_p1 < 0.3:
                factors.append(f"{match.p2_name} domina el H2H ({int((1-h2h_p1)*h2h_total)}/{h2h_total})")
        
        # First serve advantage
        p1_serve = features.get("p1_first_serve_pct_10", 0)
        p2_serve = features.get("p2_first_serve_pct_10", 0)
        if p1_serve > 0.65:
            factors.append(f"{match.p1_name} tiene buen primer servicio ({p1_serve*100:.0f}%)")
        if p2_serve > 0.65:
            factors.append(f"{match.p2_name} tiene buen primer servicio ({p2_serve*100:.0f}%)")
        
        return factors[:5]

    def generate_tennis_markets(self, match: TennisMatch, p1_prob: float, p2_prob: float) -> list:
        ...
        if p1_prob > p2_prob:
            markets.append({
                ...
                "confidence_level": "high" if p1_prob > 0.65 else "medium" if p1_prob > 0.55 else "low",
            ...
            markets.append({
                ...
                "confidence_level": "high" if p2_prob > 0.65 else "medium" if p2_prob > 0.55 else "low",
        ...
        markets.append({
            ...
            "confidence_level": "high" if over_prob > 0.6 else "medium" if over_prob > 0.5 else "low",
        ...
        markets.append({
            ...
            "confidence_level": "high" if under_prob > 0.6 else "medium" if under_prob > 0.5 else "low",
