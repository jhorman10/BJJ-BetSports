...
class BasketballFeatureExtractor:
    ...
    CONFERENCES = {
        "East": ["BOS", "NYK", "PHI", "MIL", "CLE", "IND", "MIA", "CHI",
                 "ATL", "BKN", "TOR", "WAS", "DET", "CHA", "ORL"],
        "West": ["DEN", "OKC", "MIN", "LAC", "DAL", "PHX", "LAL", "SAC",
                 "GSW", "NOP", "HOU", "SAS", "POR", "UTA", "MEM"],
    }
    ...
    def _compute_team_history(self):
        ...
        for _, row in self.historical_data.iterrows():
            ...
            self.team_history[home].append({
                "won": home_win, "pts_scored": home_score,
                "pts_allowed": away_score, "date": game_date, "is_home": 1,
            })
            self.team_history[away].append({
                "won": 1 - home_win, "pts_scored": away_score,
                "pts_allowed": home_score, "date": game_date, "is_home": 0,
            })

    def _compute_h2h(self):
    ...
    def _get_recent_team_stats(self, team: str, window: int = 10,
                                before_date: Optional[date] = None) -> Dict[str, float]:
        """Get rolling stats for a team over their last N games."""
        history = self.team_history.get(team, [])
        if before_date is not None:
            history = [g for g in history if g["date"] is not None and g["date"] < before_date]
        recent = history[-window:] if len(history) > 0 else []
        if not recent:
            return {
                "win_rate": 0.5, "pts_scored_avg": 112.0, "pts_allowed_avg": 112.0,
                "home_win_rate": 0.550, "away_win_rate": 0.450,
            }
        total = len(recent)
        ...
        return {
            ...
            "home_win_rate": sum(g["won"] for g in home_games) / len(home_games) if home_games else 0.550,
            "away_win_rate": sum(g["won"] for g in away_games) / len(away_games) if away_games else 0.450,
        }
    ...
    def get_h2h_features(self, team1: str, team2: str) -> Dict[str, float]:
        ...
        total = stats["total"]
        if total == 0:
            return {"h2h_total": 0, "h2h_team1_win_rate": 0.5, "h2h_team2_win_rate": 0.5}
    ...
    def extract_features(self, game: BasketballGame) -> Dict[str, float]:
        ...
        home_form = self._get_recent_team_stats(game.home_team, window=10, before_date=game.game_date)
        away_form = self._get_recent_team_stats(game.away_team, window=10, before_date=game.game_date)
        features["home_pts_avg"] = game.home_pts_avg if game.home_pts_avg is not None else home_form["pts_scored_avg"]
        features["away_pts_avg"] = game.away_pts_avg if game.away_pts_avg is not None else away_form["pts_scored_avg"]
        features["home_fg_pct"] = game.home_fg_pct if game.home_fg_pct is not None else 0.465
        features["away_fg_pct"] = game.away_fg_pct if game.away_fg_pct is not None else 0.465
        features["home_3pt_pct"] = game.home_3pt_pct if game.home_3pt_pct is not None else 0.360
        features["away_3pt_pct"] = game.away_3pt_pct if game.away_3pt_pct is not None else 0.360
        ...
        features["home_pts_allowed_avg"] = game.home_pts_allowed_avg if game.home_pts_allowed_avg is not None else home_form["pts_allowed_avg"]
        features["away_pts_allowed_avg"] = game.away_pts_allowed_avg if game.away_pts_allowed_avg is not None else away_form["pts_allowed_avg"]
        features["home_def_rating"] = game.home_def_rating if game.home_def_rating is not None else 112.0
        features["away_def_rating"] = game.away_def_rating if game.away_def_rating is not None else 112.0
        features["def_rating_diff"] = features["home_def_rating"] - features["away_def_rating"]
        features["home_net_rating"] = (game.home_off_rating if game.home_off_rating is not None else 112.0) - features["home_def_rating"]
        features["away_net_rating"] = (game.away_off_rating if game.away_off_rating is not None else 112.0) - features["away_def_rating"]
        ...
        features["home_off_rating"] = game.home_off_rating if game.home_off_rating is not None else 112.0
        features["away_off_rating"] = game.away_off_rating if game.away_off_rating is not None else 112.0
        features["off_rating_diff"] = features["home_off_rating"] - features["away_off_rating"]

        # Group 4: Recent Form (6 features)
        home_20 = self._get_recent_team_stats(game.home_team, window=20, before_date=game.game_date)
        away_20 = self._get_recent_team_stats(game.away_team, window=20, before_date=game.game_date)
        ...
        features["is_conference"] = 1.0 if self._is_conference(game.home_team, game.away_team) else 0.0
        features["home_rest_days"] = float(game.home_rest_days) if game.home_rest_days is not None else 1.0
        features["away_rest_days"] = float(game.away_rest_days) if game.away_rest_days is not None else 1.0
        ...
        if game.home_odds is not None and game.away_odds is not None:
            ...
            features["odds_diff"] = features["home_implied_prob"] - features["away_implied_prob"]
        else:
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            # Group 1: Offense (8)
            "home_pts_avg", "away_pts_avg", "home_fg_pct", "away_fg_pct",
            "home_3pt_pct", "away_3pt_pct", "pts_avg_diff", "fg_pct_diff",
            # Group 2: Defense (8)
            "home_pts_allowed_avg", "away_pts_allowed_avg", "home_def_rating",
            "away_def_rating", "def_rating_diff", "home_net_rating",
            "away_net_rating", "net_rating_diff",
            # Group 3: Pace & Efficiency (6)
            "home_pace", "away_pace", "pace_diff", "home_off_rating",
            "away_off_rating", "off_rating_diff",
            # Group 4: Recent Form (6)
            "home_win_rate_10", "away_win_rate_10", "home_win_rate_20",
            "away_win_rate_20", "home_home_win_rate", "away_away_win_rate",
            # Group 5: H2H (3)
            "h2h_total", "h2h_home_win_rate", "h2h_away_win_rate",
            # Group 6: Context (6)
            "is_conference", "home_rest_days", "away_rest_days",
            "is_b2b_home", "is_b2b_away", "rest_diff",
            # Group 7: Betting Odds (3)
            "home_implied_prob", "away_implied_prob", "odds_diff",
        ]
