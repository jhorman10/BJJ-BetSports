...
class BaseballFeatureExtractor:
    ...
    DIVISIONS = {
        "AL": ["NYY", "BOS", "TB", "BAL", "TOR", "HOU", "SEA", "OAK", "TEX", "LAA", "CWS", "MIN", "DET", "CLE", "KC"],
        "NL": ["LAD", "SF", "SD", "ARI", "COL", "ATL", "PHI", "NYM", "WSH", "MIA", "CHC", "MIL", "STL", "PIT", "CIN"],
    }
    ...
    def _compute_team_history(self):
        ...
        for _, row in self.historical_data.iterrows():
            ...
            self.team_history[home].append({
                "won": home_win, "runs_scored": home_score,
                "runs_allowed": away_score, "date": game_date, "is_home": 1,
            })
            self.team_history[away].append({
                "won": 1 - home_win, "runs_scored": away_score,
                "runs_allowed": home_score, "date": game_date, "is_home": 0,
            })
    ...
    def _get_recent_team_stats(self, team: str, window: int = 10,
        ...
        if not recent:
            return {
                "win_rate": 0.5, "runs_scored_avg": 4.5, "runs_allowed_avg": 4.5,
                "home_win_rate": 0.538, "away_win_rate": 0.462,
            }
        total = len(recent)
        ...
        return {
            ...
            "home_win_rate": sum(g["won"] for g in home_games) / len(home_games) if home_games else 0.5,
            "away_win_rate": sum(g["won"] for g in away_games) / len(away_games) if away_games else 0.5,
        }
    ...
    def extract_features(self, game: BaseballGame) -> Dict[str, float]:
        ...
        features["pitcher_matchup_known"] = 1.0 if (game.home_pitcher_name and game.away_pitcher_name) else 0.0
        ...
        home_bat = self._get_recent_team_stats(game.home_team, window=10, before_date=game.date)
        away_bat = self._get_recent_team_stats(game.away_team, window=10, before_date=game.date)
        ...
        features["run_diff_home"] = home_bat["runs_scored_avg"] - home_bat["runs_allowed_avg"]
        features["run_diff_away"] = away_bat["runs_scored_avg"] - away_bat["runs_allowed_avg"]
        features["run_diff_diff"] = features["run_diff_home"] - features["run_diff_away"]
        ...
        home_20 = self._get_recent_team_stats(game.home_team, window=20, before_date=game.date)
        away_20 = self._get_recent_team_stats(game.away_team, window=20, before_date=game.date)
        ...
        features["is_divisional"] = 1.0 if self._is_divisional(game.home_team, game.away_team) else 0.0
        features["is_interleague"] = 1.0 if self._is_interleague(game.home_team, game.away_team) else 0.0
    ...
    def _is_interleague(self, team1: str, team2: str) -> bool:
        ...
        return (team1 in al_teams and team2 in nl_teams) or (team1 in nl_teams and team2 in al_teams)

    def get_feature_names(self) -> List[str]:
        """Return all 46 feature names in order."""
        return [
            # Group 1: Pitcher Matchup (8)
            "home_pitcher_available", "away_pitcher_available", "pitcher_matchup_known",
            "home_pitcher_era", "away_pitcher_era", "home_pitcher_whip",
            "away_pitcher_whip", "pitcher_era_diff",
            # Group 2: Team Batting (10)
            "home_runs_scored_avg", "away_runs_scored_avg", "home_runs_allowed_avg",
            "away_runs_allowed_avg", "run_diff_home", "run_diff_away", "run_diff_diff",
            "home_batting_avg", "away_batting_avg", "ops_diff",
            # Group 3: Bullpen/Pitching (8)
            "home_era", "away_era", "home_whip", "away_whip",
            "era_diff", "whip_diff", "home_bullpen_era", "away_bullpen_era",
            ...
            # Group 6: Context (8)
            "is_day_game", "is_divisional", "is_interleague",
            "home_rest_days", "away_rest_days", "home_wins", "away_wins", "home_losses",
