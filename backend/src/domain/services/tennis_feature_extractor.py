...
class TennisFeatureExtractor:
    ...
    LEVELS = ['G', 'A', 'M', 'D', 'F']  # Grand Slam, ATP, Masters, Davis Cup, Finals
    ...
    def __init__(self, historical_data: Optional[pd.DataFrame] = None):
        ...
        self.player_stats = {}  # Cache for player rolling stats
        self.h2h_stats = {}     # Cache for H2H stats
        self.player_history = defaultdict(list)  # Per-player chronological match list
    ...
    def _compute_rolling_history(self):
        """
        ...
        for _, row in self.historical_data.iterrows():
            ...
            bp_converted = ((w_bpFaced - w_bpSaved) / w_bpFaced) if w_bpFaced > 0 else 0.0

            self.player_history[winner].append({
                'won': 1, 'aces': w_ace, 'dfs': w_df,
            ...
            bp_converted_l = ((l_bpFaced - l_bpSaved) / l_bpFaced) if l_bpFaced > 0 else 0.0

            self.player_history[loser].append({
                'won': 0, 'aces': l_ace, 'dfs': l_df,
    ...
    def _get_recent_matches(self, player_name: str, window: int = 10,
                            before_date: Optional[date] = None) -> List[dict]:
        """
        ...
        if before_date is not None:
            ...
            filtered = [m for m in history if m['date'] is not None and m['date'] < before_date]
        else:
            filtered = history

        return filtered[-window:] if len(filtered) > 0 else []

    def _get_rolling_stats(self, player_name: str, window: int = 10,
    ...
    def _get_surface_win_rate(self, player_name: str, surface: str,
                              before_date: Optional[date] = None) -> float:
        """Get win rate for a player on a specific surface."""
        history = self.player_history.get(player_name, [])

        if before_date is not None:
            history = [m for m in history if m['date'] is not None and m['date'] < before_date]
        ...
        return sum(m['won'] for m in surface_matches) / len(surface_matches)
    ...
    def extract_features(self, match: TennisMatch) -> Dict[str, float]:
        ...
        features['rank_points_diff'] = (match.p1_rank_points or 0) - (match.p2_rank_points or 0)
        ...
        p1_10 = self._get_rolling_stats(match.p1_name, window=10, before_date=match.match_date)
        p2_10 = self._get_rolling_stats(match.p2_name, window=10, before_date=match.match_date)
        ...
        p1_20 = self._get_rolling_stats(match.p1_name, window=20, before_date=match.match_date)
        p2_20 = self._get_rolling_stats(match.p2_name, window=20, before_date=match.match_date)
        features['p1_win_rate_20'] = p1_20['win_rate']
        features['p2_win_rate_20'] = p2_20['win_rate']

        # 8. Surface-specific win rate
        features['p1_surface_win_rate'] = self._get_surface_win_rate(match.p1_name, match.surface, before_date=match.match_date)
        features['p2_surface_win_rate'] = self._get_surface_win_rate(match.p2_name, match.surface, before_date=match.match_date)
        ...
        if match.p1_odds is not None and match.p2_odds is not None:
            ...
            features['odds_diff'] = features['p1_implied_prob'] - features['p2_implied_prob']
        else:
    ...
    def get_feature_names(self) -> List[str]:
        ...
        rolling_features_10 = [
            ...
            'p1_ace_rate_10', 'p2_ace_rate_10',
            'p1_df_rate_10', 'p2_df_rate_10',
            'p1_first_serve_pct_10', 'p2_first_serve_pct_10',
            'p1_break_point_conv_10', 'p2_break_point_conv_10',
        ]
        ...
        odds_features = [
            'p1_implied_prob', 'p2_implied_prob', 'odds_diff',
        ]

        return (base_features + surface_features + level_features
                + rolling_features_10 + rolling_features_20
                + surface_win_rate + odds_features)
