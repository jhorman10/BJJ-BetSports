        ...
        try:
            ...
            if os.getenv("DISABLE_ML_TRAINING") == "true":
                ...
                training_result = await orchestrator.run_training_pipeline(
                    league_ids=leagues, days_back=550
                )
            ...
            for league_id in DEFAULT_LEAGUES:
                try:
                    ...
                    # 1. Ephemeral Cache
                    cache.set(
                        league_cache_key, predictions_dto.dict(), cache.TTL_FORECASTS
                    )

                    # 2. Persistent Storage (PostgreSQL)
                    if persistence_repo:
                        persistence_repo.save_training_result(
                            league_cache_key, predictions_dto.dict()
                        )
            ...
            logger.error(
                f"CRITICAL Error during orchestrated job: {str(e)}", exc_info=True
            )
            gc.collect()
        finally:
            self._job_in_progress = False
