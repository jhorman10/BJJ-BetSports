            ...
            for i in range(retries):
                try:
                    ...
                    if i == retries - 1:
                        ...
                        raise
                    logger.warning(
                        "Retry %d/%d for %s due to %s. Waiting %ss...",
                        i + 1,
                        retries,
                        func.__name__,
                        e,
                    ...
                    logger.error("Non-retriable error in %s: %s", func.__name__, e)
                    raise
            return None
...
class FootballDataUKSource:
    ...
    # Whitelist of league codes known to provide valid .csv files
    # at /mmz4281/{season}/{code}.csv
    ...
    SUPPORTED_LEAGUES = {
        ...
        "SC0",
        "SC1",  # Scotland (Prem, Div 1) - Optional, adding just in case
        "T1",  # Turkey
        "G1",  # Greece
    }
    ...
    async def download_csv(
        ...
        try:
            ...
            if os.getenv("API_ONLY_MODE", "false").lower() == "true":
                logger.warning(
                    "Skipping CSV download for %s: Pandas missing.", league_code
                )
                return None
            else:
                raise ImportError(
                    "Pandas is required for CSV processing but is not installed."
                )
        ...
        try:
            ...
            if "Date" in df.columns and not df.empty:
                try:
                    ...
                    logger.debug("Could not extract latest date from CSV: %s", e)

            logger.info(
                "Downloaded %d matches from %s. Latest match date in CSV: %s",
                len(df),
                url,
                latest_date,
            )
            ...
            if e.response.status_code == 404:
                logger.warning(
                    "CSV not found (404) for %s; season may not be available.",
                    url,
                )
                return None
            raise  # Let retry decorator handle other status codes
        except Exception as e:
            logger.error("Error processing CSV from %s: %s", url, e)
            return None
    ...
    def parse_matches(
        ...
        if not all(col in df.columns for col in required_cols):
            logger.warning(
                f"Missing required columns in data. Available: {df.columns.tolist()}"
            )
            return matches
        ...
        for idx, row in df.iterrows():
            try:
                ...
                home_corners = (
                    int(row["HC"]) if "HC" in row and pd.notna(row["HC"]) else None
                )
                away_corners = (
                    int(row["AC"]) if "AC" in row and pd.notna(row["AC"]) else None
                )

                # HY = Home Yellows, AY = Away Yellows
                home_yellow = (
                    int(row["HY"]) if "HY" in row and pd.notna(row["HY"]) else None
                )
                away_yellow = (
                    int(row["AY"]) if "AY" in row and pd.notna(row["AY"]) else None
                )

                # HR = Home Reds, AR = Away Reds
                home_red = (
                    int(row["HR"]) if "HR" in row and pd.notna(row["HR"]) else None
                )
                away_red = (
                    int(row["AR"]) if "AR" in row and pd.notna(row["AR"]) else None
                )

                # EXTENDED STATS (Shots, Target, Fouls)
                # HS = Home Shots, AS = Away Shots
                home_shots = (
                    int(row["HS"]) if "HS" in row and pd.notna(row["HS"]) else None
                )
                away_shots = (
                    int(row["AS"]) if "AS" in row and pd.notna(row["AS"]) else None
                )

                # HST = Home Shots on Target, AST = Away Shots on Target
                home_shots_on_target = (
                    int(row["HST"]) if "HST" in row and pd.notna(row["HST"]) else None
                )
                away_shots_on_target = (
                    int(row["AST"]) if "AST" in row and pd.notna(row["AST"]) else None
                )

                # HF = Home Fouls, AF = Away Fouls
                home_fouls = (
                    int(row["HF"]) if "HF" in row and pd.notna(row["HF"]) else None
                )
                away_fouls = (
                    int(row["AF"]) if "AF" in row and pd.notna(row["AF"]) else None
                )
    ...
    async def get_historical_matches(
        ...
        if seasons is None:
            ...
            try:
                ...
                logger.warning(
                    "Failed to compute previous season from %s: %s", current_season, exc
                )
                seasons = [current_season, "2324"]

            logger.info("Targeting seasons for %s: %s", league_code, seasons)
        ...
        if league_code not in self.SUPPORTED_LEAGUES:
            # Silent return or debug to avoid spamming warnings
            # for expected unsupported leagues (like Cups)
            logger.debug(
                "Skipping %s: Not in Football-Data.co.uk supported whitelist",
                league_code,
            )
            return []
        ...
        async with httpx.AsyncClient() as client:
            tasks = [
                self.download_csv(
                    league_code, season, force_refresh=force_refresh, client=client
                )
                for season in seasons
            ]
            results = await asyncio.gather(*tasks)

        # Note: tracking current-season load success was removed; we keep logging
            ...
            if result is not None:
                ...
                if not df.empty:
                    ...
                    if i == 0:  # Current season
                        logger.info(
                            "Successfully loaded %d matches for %s season %s",
                            len(matches),
                            league_code,
                            season_code,
                        )
                ...
                if i == 0:
                    logger.warning(
                        "Failed to load season %s for %s; falling back to history.",
                        season_code,
                        league_code,
                    )
        ...
        if not all_matches:
            logger.error(
                "No matches found for %s in seasons %s",
                league_code,
                seasons,
            )
